...
class NearestNeighborClassifier:
    ...
    def get_nearest_neighbor(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # calculate l2 norm + use argmin.

        l2_norm = torch.linalg.vector_norm(self.data_normalized - x, ord=2, dim=1)

        nearest_neighbor = torch.argmin(l2_norm)

        return self.data[nearest_neighbor], self.label[nearest_neighbor]
    ...
    def get_k_nearest_neighbor(self, x: torch.Tensor, k: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        nearest_neighbor = torch.topk(l2_norm, k, largest=False, sorted=True)

        return self.data[nearest_neighbor[1]], self.label[nearest_neighbor[1]]
