...
class PyTorchBasics:
    ...
    def make_it_pytorch_4(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.sum(y > torch.mean(x)) # sum boolean array
    ...
    def make_it_pytorch_9(x: torch.Tensor) -> torch.Tensor:
        # used copilot to explain the question here. They pointed me to summed-area table.
        """
    ...
    def make_it_pytorch_12(x: torch.Tensor, m: torch.BoolTensor) -> torch.Tensor:
        ...
        Solution length: 11 characters
        """
        return x[torch.nonzero(m, as_tuple=True)]
    ...
    def make_it_pytorch_extra_2(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        """
        1. convert X from [D] to [1,D] -> unsqueeze
        3. Use sum + condition(touches .any under the hood I think?) 
        """
        return torch.sum(torch.abs(x.unsqueeze(1) - y) < 0.001)
